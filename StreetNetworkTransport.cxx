...
namespace Polyphemus
{
  ...
  */
  ...
  void StreetNetworkTransport<T>::ReadConfiguration()
  {
	...
	//cout << "Ns_background " << Ns_background << endl;
      }
    ...
    if (this->option_process["with_local_data"])
      {
	    ...
	    // cout << "background_tmp: " << background_tmp << endl;	   
	    //            for (int t = 0; t < Nt_emis; t++)
            for (int t = 0; t < Nt_background; t++) // !!!! YK
              for (int st = 0; st < total_nstreet; st++)
		{
		  //		  cout << s << "\t" << t << "\t" << st << "\t" << background_tmp(t, st) << endl;
                background_concentration(s, t, st) = background_tmp(t, st);
		}
          }
      }

    //    cout << "background_concentration in InitData: " << background_concentration << endl;
  ...
  */
  ...
  void StreetNetworkTransport<T>::InitStep()
  {
	...
	//cout << "background_index " << background_index << endl;
        if (background_index >= background_concentration.shape()[1])
          throw("Error: missing background concentration data.");
      }


    // cout << "background_concentration in InitStep: " << background_concentration << endl;
    ...
    for (typename vector<Street<T>* >::iterator iter = StreetVector.begin();
         iter != StreetVector.end(); iter++)
      {
        ...
        for (int i = 0; i < this->Ns; ++i)
	      ...
	      {
		//cout<<species_list_emis[j]<<" emission = "<<emission(j, emis_index, st)<<endl; 
	...
	//throw;
	    ...
	    //cout << "Call SetBackground\t" << street->GetStreetID() << endl;
            for (int i = 0; i < this->Ns; ++i)
		  ...
		  // cout << this->species_list[i] << "\t" << background_concentration(j, background_index, st) << endl;
		  // cout <<  j << "\t" << "background_index " << "\t" << background_index << "\t" << st << endl;
		  }
          }
        ++st; 
      }
  ...
  void StreetNetworkTransport<T>::ComputeStreetConcentration()
  {
    ...
    for (typename vector<Street<T>* >::iterator iter = StreetVector.begin();
         iter != StreetVector.end(); iter++)
      {
		...
		//cout << s << emission_rate << "\t" << inflow_rate << "\t" << temp * conc_bg << endl;
  ...
  */
  ...
  void StreetNetworkTransport<T>::ComputeSigmaW()
  {
	...
	//cout << "sigma_w_ = " << sigma_w_ << endl; same //lemmoucf
      }
  }
  ...
  void StreetNetworkTransport<T>::ComputeUstreet()
  {
	...
	//cout << "SNT ustreet = " << ustreet << endl; same!! //lemmoucf
	...
	for (int s = 0; s < this->Ns; s++)
	  {
	    ...
	    if (tmp != 0.0)
	      {
		sub_delta_t_sp(s) = concentration_array(s)/abs(dcdt(s));
	      }
	  }
      }
